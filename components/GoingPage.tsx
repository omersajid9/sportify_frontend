...
const getUpcomingGames = (user_id: string | undefined, location: { lat: number, lng: number }) => {
    return useQuery({
        queryKey: ['sessions', 'going', user_id], queryFn: async () => {
            const response = await axiosInstance.get('/search/going_sessions', { params: { user_id: user_id, lat: location.lat, lng: location.lng } });
...
export default function GoingPage() {
    ...
    var { data: sessions, isLoading, error, refetch } = getUpcomingGames(user?.id, Location);
